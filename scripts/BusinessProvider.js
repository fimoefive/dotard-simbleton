...
const newYorkBusiness = businesses.filter(businessObj => {
...
export const useNewYorkBusinesses = () => {
  return newYorkBusiness;
};

const manufacturingBusinesses = businesses.filter(businessObj => {
  if (businessObj.companyIndustry === "Manufacturing") {
    return true;
  }
  return false;
});
export const usemanufacturingBusinesses = () => {
  return manufacturingBusinesses;
};

const purchaserAgent = businesses.map(businessObj => {
  return `{ 
...
export const usepurchaserAgent = () => {
  return purchaserAgent;
};